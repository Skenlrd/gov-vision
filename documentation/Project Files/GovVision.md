...
MODULE 1
Decision Management & Workflow System
Arnab Roy  ·  202422025

2.1 Work to be Done — Module 1

CLIENT-SIDE TASKS

Project Setup
•	Initialise React.js project with Vite build tool; configure React Router v6 for all routes with a PrivateRoute wrapper that checks JWT validity before rendering protected pages
•	Set up global state with Redux Toolkit or Zustand: slices for auth (user, token), decisions (list, selected), and notifications (list, unread count)
•	Configure Axios instance with a base URL env variable; add request interceptor to attach Authorization: Bearer <token> to all outgoing requests; add response interceptor to handle 401 by attempting token refresh or redirecting to login
•	Build reusable UI component library: StatusBadge (colour-coded for each decision status), Modal (portal-based), DataTable (sortable/filterable columns), Pagination, FileListItem, StepperTracker (visual approval chain)
Decision Creation & Management
•	Multi-step proposal form: Step 1 — title, category dropdown (populated from workflow configs API), priority select; Step 2 — proposal body textarea with character count; Step 3 — file upload drag-and-drop zone listing attached files with remove button; Step 4 — reviewer assignment multi-select showing user name and role
•	Decision list dashboard: status filter tabs (All / Draft / Under Review / Approved / Rejected / Archived), keyword search bar, department and date-range filters, sortable table columns, click row to navigate to Decision Detail View
•	Decision detail view: summary card at top (title, category, priority, status badge, created by, submitted date); StepperTracker below showing all approval stages; comments thread with reply support; file versions list with version number, uploader, and download link; full audit history timeline at bottom
Approval Workflow UI
•	Approver dashboard: pending approvals card list, sorted by SLA due date ascending; overdue badges in red; priority colour indicators; click card to open decision detail or trigger approval modal
•	Approval action modal: overlays the current page; shows decision title and current stage; radio group with three options (Approve / Reject / Request Revision); mandatory remarks textarea (enforced minimum 20 characters client-side); Confirm and Cancel buttons
•	Real-time status updates on the Decision Detail View using 20-second polling of GET /api/decisions/:id/status — updates the StepperTracker without a full page reload
Notifications & Settings
•	Notification bell in the header navbar: shows unread count badge; on click, dropdown list of last 10 notifications with type icon, message, time elapsed, and a link to the related decision
•	Notification Centre page: full paginated list, filter by type (approval request / state change / mention / SLA warning), Mark All Read button
•	User profile page: edit name field, department display, change password form (calls PUT /api/auth/password with current + new password; server runs bcrypt.compare then bcrypt.hash)

SERVER-SIDE TASKS

Project Structure & Middleware
•	Folder structure: /controllers, /routes, /services, /models, /middleware, /utils. Each layer has a single responsibility — controllers handle HTTP in/out, services contain business logic, models define Mongoose schemas.
•	Error handling middleware: global Express error handler catches all thrown errors; returns consistent JSON { success: false, error: message, code: errorCode }
•	express-validator middleware on all POST/PUT routes: validate field types, lengths, enum values, ObjectId format; return 422 with field-level errors if validation fails
•	express-rate-limit: 100 requests per 15 minutes per IP on all routes; 5 requests per 15 minutes on /api/auth/login to prevent brute force
•	CORS restricted to the frontend origin domain via cors npm package; all other origins blocked
•	Helmet.js: set HTTP security headers (X-Frame-Options, Content-Security-Policy, X-Content-Type-Options, etc.)
Decision Lifecycle Engine
•	State machine implementation: a validateTransition(currentStatus, requestedAction, userRole) function is called before every status change. It enforces allowed transitions: only Owner can submit Draft; only the current-stage Reviewer can approve/reject/revise; only Admin can archive.
•	On submission: read m1_workflow_configs by decision category; build the approvalChain array ordered by stage number; set currentStage = 0; update status to 'submitted'; call Notification Service to alert the Stage 0 approver
•	On approval: increment currentStage; if currentStage equals approvalChain.length, set status = 'approved' and set completedAt = now; else notify the next stage approver
•	On rejection: set status = 'rejected', set completedAt = now, notify the decision creator
•	On revision request: set status = 'revision_requested', notify creator with reviewer remarks; creator may re-edit and re-submit (which resets the chain from stage 0)
Escalation Processor
•	node-cron job runs every 60 minutes; queries all decisions where status = 'under_review' AND slaDeadline < now AND no escalation notification sent in the past 24 hours
•	For each overdue decision, sends an escalation in-app notification to the current-stage approver and a separate notification to the department head (fetched from departments.head FK)
File Management Service
•	Accept multipart/form-data via multer middleware; validate MIME type against allowed list (application/pdf, application/vnd.openxmlformats-officedocument.*); enforce 10 MB file size limit
•	Save file to ./uploads directory or MongoDB GridFS; create a new m1_file_versions document with decisionId, fileName, storagePath, versionNumber (auto-incremented per decision), and uploadedBy
•	Secure download: GET /api/decisions/:id/files/:fileId verifies the requesting user has read access to the decision before streaming the file
Comment & Notification Service
•	Comment creation: parse text for @username patterns; for each mention, resolve the username to a userId from the users collection and create a 'mention' notification for that user
•	Notification writer: called internally on every state transition, file upload, comment, and SLA warning; creates one m1_notifications document per recipient
•	Email notification: Nodemailer is called for approval_request, state_change, and SLA_warning notification types; email template includes decision title, current status, and a direct link to the decision URL
Audit Bridge
•	After every decision state transition, call Module 2's internal endpoint POST /api/internal/audit/log via Axios with SERVICE_KEY header; payload includes: userId, action (e.g. DECISION_APPROVED), entityType: 'decision', entityId: decision._id, module: 'm1', before snapshot, after snapshot, and the requesting client IP
•	Webhook Behavior: decision-update webhook triggers KPI recomputation AND real-time anomaly detection via FastAPI
•	Morgan logger: log all incoming HTTP requests (method, URL, status code, response time) to stdout
•	Winston logger: structured JSON logging for all errors, state transition events, and escalation actions to a daily rotating log file

2.2 Pages to be Created — Module 1

Module 1 has 11 pages. All are React.js routes protected by JWT-based PrivateRoute guards. Role-based rendering controls which action buttons appear per page.

Page / Screen	Purpose	UI Components	User Actions	Data Displayed
Login	Authenticate user; issue JWT session	Email input, Password input, Submit button, Forgot Password link, Error banner	Submit credentials, navigate to Forgot Password	Error messages; redirect to dashboard on success
Register	Create new user account	Name input, Email input, Password input, Confirm Password, Department dropdown, Role select (Admin-only), Submit button	Fill all fields, submit; Admin assigns role	Inline validation errors; success redirect
Decision Maker Dashboard	Personal overview of all decisions created by the logged-in user	KPI cards (Total / Draft / Pending / Approved / Rejected), Decisions table with StatusBadge, Quick-Create button, Status filter tabs, Keyword search, Date range picker, Department filter	Create new decision, filter by status/date/department, search, click row for detail	Decision title, category, priority badge, status badge, submit date, current stage, SLA days remaining
Create Decision Proposal	Multi-step form to draft and submit a new decision proposal	4-step progress indicator, Title input, Category select, Priority select, Body textarea (Step 2), Drag-and-drop file upload zone with file list (Step 3), Reviewer multi-select with role labels (Step 4), Save Draft button, Submit button	Complete all steps, attach files, assign reviewers, save draft at any step, submit for review	Step validation errors, attached file names and sizes, selected reviewer names and roles
Decision Detail View	Full read/write view of one decision — the primary workspace for all stakeholders	Summary header card (title, meta, StatusBadge), StepperTracker (approval chain visual), Action panel (Approve/Reject/Revise — visible only to current-stage reviewer), Comments thread with reply and mention support, File versions list with download links, Audit history timeline	Add comment, reply to comment, @mention a user, download any file version, take approval action (reviewer only), view full audit trail	All decision metadata, approval chain with per-stage status and reviewer remarks, all comments, all file versions, full audit history
Approver Dashboard	Queue of decisions awaiting the logged-in reviewer's action	Pending approval cards sorted by SLA due date, Priority colour indicators, Overdue red badge, Category and Department filter dropdowns, Sort controls	Open decision detail, trigger Approval Action Modal from card, filter and sort queue	Decision title, submitter name, submission date, SLA deadline countdown, category, priority, current stage label
Approval Action Modal	Focused inline overlay to take an approval action without navigating away	Decision title display, Action radio group (Approve / Reject / Request Revision), Mandatory remarks textarea, Character count display, Confirm button, Cancel button	Select action, write remarks (required), confirm or cancel	Decision title, current stage name, previous reviewer remarks
Decision Archive	Read-only list of all completed and archived decisions	Searchable/filterable table (title, final status, resolution date, department, resolved by), Export to CSV button, Column sort controls	Search, filter by status/department/date range, export list, click row to open read-only detail	Title, final status badge, resolved date, resolved by user, department, decision category
Notification Centre	Full list of all system notifications for the logged-in user	Notification list rows (type icon, message, timestamp, linked decision title, read/unread dot), Mark All Read button, Filter by type dropdown, Pagination	Click notification to navigate to linked decision, mark individual notification read, mark all read, filter by type	Notification type, message, linked decision title, time since creation, read/unread status
Admin — User Management	Admin panel for creating and managing all platform users	User table (name, email, role badge, department, account status, last login), Add User modal (name/email/password/role/dept), Edit Role dropdown in row, Activate/Deactivate toggle, Search bar	Create user, edit role, activate/deactivate account, search by name or email	All users with role, department, account status, and last login timestamp
Admin — Workflow Config	Configure approval chains and SLA rules per decision category	Decision category list on left, selected category's approval chain builder on right (ordered list with Add/Remove/Reorder stage controls, each stage: role select or specific user select), SLA days input, Save button	Add new decision category, define approval chain stages by assigning roles or users, set SLA days, save configuration	All configured categories, their ordered approval chains, SLA values; unsaved changes indicator

2.3 Database Design — Module 1

Module 1 owns six collections and shares the users and departments collections with all other modules. PK = Primary Key (auto-generated ObjectId). FK = Foreign Key reference to another collection's _id.

Collection: users  [SHARED — all modules]
Field	Type	Constraints	Description
PK _id	ObjectId	Auto-generated, Unique	Unique user identifier; referenced as FK across all modules
name	String	Required, max 100 chars	Full name shown in UI, notifications, and reports
email	String	Required, Unique, Indexed	Login credential; used for email notifications
passwordHash	String	Required	bcrypt hash (cost=10); plain text never stored
role	String	Required, Indexed	admin|manager|decision_maker|reviewer|compliance_officer|analyst|employee|viewer
department	String	Indexed	Department name; used for scoped analytics in Module 3
isActive	Boolean	Default: true	Soft-disable; inactive users cannot log in or receive assignments
notifPrefs	Object		{ emailEnabled: Boolean, inAppEnabled: Boolean, types: [String] }
createdAt	Date	Auto (Mongoose timestamp)	
updatedAt	Date	Auto (Mongoose timestamp)	

Collection: departments  [SHARED — all modules]
Field	Type	Constraints	Description
PK _id	ObjectId	Auto-generated	Unique department ID
name	String	Required, Unique	Department display name
FK head	ObjectId	Ref: users	Department head; used for escalation notifications in Module 1
isActive	Boolean	Default: true	

Collection: m1_decisions
Field	Type	Constraints	Description
PK _id	ObjectId	Auto, Indexed	Unique decision ID; referenced by m2_violations.decisionId and m3_anomalies.decisionId
title	String	Required, Indexed	Short decision title
body	String	Required	Full proposal content
category	String	Required, Indexed	Matched against m1_workflow_configs.category for chain lookup
priority	String	Required	low | medium | high | critical
status	String	Required, Indexed	draft | submitted | under_review | approved | rejected | revision_requested | archived
FK createdBy	ObjectId	Ref: users, Indexed	Decision author / owner
FK department	ObjectId	Ref: departments, Indexed	Owning department; used by Module 3 for KPI grouping
approvalChain	Array		[{ FK approver: ObjectId(ref:users), order: Number, status: String, actedAt: Date, remarks: String }]
currentStage	Number	Default: 0	Index into approvalChain array; drives routing logic
slaDeadline	Date	Set on submission	Auto-computed from m1_workflow_configs.slaDays
completedAt	Date	Nullable	Set when final decision (approved/rejected) is reached
createdAt	Date	Auto	
updatedAt	Date	Auto	

Collection: m1_comments
Field	Type	Constraints	Description
PK _id	ObjectId	Auto	
FK decisionId	ObjectId	Ref: m1_decisions, Indexed	Parent decision
FK author	ObjectId	Ref: users	Comment author
text	String	Required	Comment body text
FK parentId	ObjectId	Ref: m1_comments, Nullable	For threaded replies; null = top-level comment
mentions	Array		[ObjectId] ref: users — mentioned users each receive a notification
isResolved	Boolean	Default: false	
createdAt	Date	Auto	

Collection: m1_notifications
Field	Type	Constraints	Description
PK _id	ObjectId	Auto	
FK userId	ObjectId	Ref: users, Indexed	Notification recipient
type	String	Indexed	approval_request | state_change | comment_mention | sla_warning | revision_request
FK decisionId	ObjectId	Ref: m1_decisions	Linked decision for navigation
message	String	Required	Notification display text
isRead	Boolean	Default: false, Indexed	
createdAt	Date	Auto, Indexed	Sorted descending for notification feed

Collection: m1_file_versions
Field	Type	Constraints	Description
PK _id	ObjectId	Auto	
FK decisionId	ObjectId	Ref: m1_decisions, Indexed	Parent decision
fileName	String	Required	Original uploaded filename
storagePath	String	Required	Filesystem path or GridFS ID
versionNumber	Number	Required	Auto-incremented per decision (1, 2, 3...)
FK uploadedBy	ObjectId	Ref: users	
uploadedAt	Date	Auto	

Collection: m1_workflow_configs
Field	Type	Constraints	Description
PK _id	ObjectId	Auto	
category	String	Required, Unique	Decision category name; looked up on submission
approvalChainDef	Array		[{ role: String, order: Number }] — defines required approval stages by role
slaDays	Number	Required	Number of calendar days allowed for full approval cycle
createdAt	Date	Auto	

Module 1 Cross-Module Relationships
•	m1_decisions._id → stored as decisionId in m2_violations when Module 2 detects a policy breach on that decision
•	m1_decisions._id → stored as decisionId in m3_anomalies when Module 3's Isolation Forest flags an anomaly on that decision
•	m1_decisions.department → shared FK with departments._id; same field used by m2_violations.department and m3_kpi_snapshots.department for three-module join on department
•	m2_audit_logs receives entries written by Module 1 on every decision state transition (via service-to-service REST call)
•	m3_kpi_snapshots is updated immediately after each decision state transition via the Module 3 webhook

2.4 REST API — Module 1

Auth Endpoints
Method	Endpoint	Description	Request Body	Response | Auth
POST	/api/auth/register	Register new user	{ name, email, password, role, department }	201 { user, accessToken } | Public
POST	/api/auth/login	Login; bcrypt verify; issue JWT pair	{ email, password }	200 { accessToken, refreshToken } | Public
POST	/api/auth/refresh	Refresh expired access token	{ refreshToken }	200 { accessToken } | Public
POST	/api/auth/logout	Invalidate refresh token	{ refreshToken }	204 No Content | JWT
PUT	/api/auth/password	Change password (bcrypt re-hash)	{ currentPassword, newPassword }	200 { message } | JWT

Decision CRUD & Lifecycle Endpoints
Method	Endpoint	Description	Request Body	Response | Auth
GET	/api/decisions	List decisions (role-scoped)	Query: status, dept, page, limit, search	200 { decisions[], total } | JWT
POST	/api/decisions	Create decision draft	{ title, body, category, priority }	201 { decision } | JWT + decision_maker
GET	/api/decisions/:id	Full decision detail with comments, files, history	Param: id	200 { decision, comments, files, history } | JWT
PUT	/api/decisions/:id	Update draft fields	{ title, body, category, priority }	200 { decision } | JWT + owner
POST	/api/decisions/:id/submit	Submit draft; build approval chain	{}	200 { decision } | JWT + owner
POST	/api/decisions/:id/approve	Approve current stage	{ remarks }	200 { decision } | JWT + reviewer
POST	/api/decisions/:id/reject	Reject decision	{ reason }	200 { decision } | JWT + reviewer
POST	/api/decisions/:id/revise	Request revision	{ notes }	200 { decision } | JWT + reviewer
GET	/api/approvals/pending	My pending approval queue	Query: page, limit	200 { approvals[] } | JWT + reviewer

Files, Comments & Notification Endpoints
Method	Endpoint	Description	Request Body	Response | Auth
POST	/api/decisions/:id/files	Upload attachment; create version record	FormData: file (binary)	201 { fileVersion } | JWT + owner
GET	/api/decisions/:id/files	List all file versions	Param: id	200 { files[] } | JWT
GET	/api/decisions/:id/files/:fid	Download a specific file version	Param: id, fid	200 File Binary | JWT
POST	/api/decisions/:id/comments	Add comment or reply	{ text, parentId? }	201 { comment } | JWT
GET	/api/decisions/:id/comments	Get full comment thread	Param: id	200 { comments[] } | JWT
GET	/api/notifications	Get user notifications	Query: read, type, page	200 { notifications[], unreadCount } | JWT
PUT	/api/notifications/read-all	Mark all notifications read	{}	200 { message } | JWT
 
MODULE 2
Governance, Policy & Compliance Engine
Moyuk Rudra  ·  202422016

3.1 Work to be Done — Module 2

CLIENT-SIDE TASKS

Policy Management UI
•	Policy library page (HTML + CSS + JavaScript): card grid layout displaying all policies; each card shows title, category badge, version number, effective date, and status label; filter controls: search text input, category dropdown, status filter buttons; Create Policy button visible to admin/manager roles only
•	Create/Edit policy form: title text input, category select, effective date date-picker, department multi-select checkbox list (populated from departments API), policy body textarea for full policy text, version notes textarea (shown only on edit), Save Draft button, Submit for Review button, Cancel button
•	Policy version history panel on the edit/detail page: ordered list showing each version's number, date, changed-by name, and version note; click a version to load that version's content into a read-only text pane alongside the current version for comparison
•	Policy detail view: full policy text display, metadata header, assigned departments list, compliance rate badge (pulled from Module 2 compliance score endpoint), Download PDF button, Edit and Archive buttons for admin only
Compliance Monitoring UI
•	Compliance dashboard: overall compliance percentage KPI card at top; per-department compliance table with department name, compliance %, violations this month, and a colour-coded status indicator (green/amber/red); violations-today counter; recent violations feed showing last 10 with severity badge and policy name
•	Violation registry page: HTML table with filter form at top (severity multi-select, department dropdown, status dropdown, date range inputs); each row: violation ID, policy name, linked decision title, department, severity badge (colour-coded), status, detected date, Resolve button
•	Violation detail page: violation metadata header, linked policy card (title, category, effective date), linked decision card (title, submitter, current status), evidence description section, resolution form (status dropdown, notes textarea, assign owner user-search input), comment thread section, audit history for this violation
Audit Trail UI
•	Audit log viewer page: advanced filter form (user dropdown populated from users API, action type multi-select, entity type select, module filter, date-from and date-to date inputs); results table below (user name, action, entity type, entity ID truncated, module, timestamp, IP); Export CSV button; pagination controls
•	Audit entry detail panel (opens as side-panel or new page): shows user info, action label, timestamp, IP address, before-state JSON block (monospace), after-state JSON block (monospace), tamper hash string with a Verified badge icon
Risk Register UI
•	Risk register page: table with columns for title, category, likelihood (1-5), impact (1-5), composite score badge (colour-coded: 1-5 Low, 6-12 Medium, 13-18 High, 19-25 Critical), owner name, status, last-updated date; Add Risk button opens an inline row form; Edit button per row; Delete (admin only)
•	Compliance reports page: parameter form (date-from/to, department multi-select, format radio buttons PDF/CSV), Generate button, report preview area showing estimated row count, Past Reports table with date, description, format badge, and Download button

SERVER-SIDE TASKS

Policy Service
•	Policy CRUD with automatic versioning: every PUT /api/policies/:id first copies the current document content into a new m2_policy_versions record (incrementing versionNumber), then updates the parent m2_policies document with the new content; this ensures complete version history is preserved
•	Policy publication state machine: Draft → Under Review → Published → Archived; state transitions validated by a policyTransition(currentStatus, action, userRole) function; only admin/manager may publish; effectiveDate scheduling via node-cron checks every morning and auto-activates policies whose effectiveDate has arrived
•	Policy assignment: assignedDepts array on each policy document is updated via a dedicated PUT /api/policies/:id/assign-departments endpoint; this array is read by the compliance validation hook
Compliance Validation Hook
•	POST /api/internal/compliance/validate (SERVICE_KEY auth): receives { decisionId, category, department } from Module 1 on every decision submission
•	Queries all m2_policies where status = 'published' AND assignedDepts includes the decision's department AND (category matches OR category is blank meaning applies to all)
•	For each matching policy, evaluates the policy's rules against the decision metadata (category, priority, completeness). If a rule is breached, records a new m2_violations document with policyId, decisionId, department, auto-computed severity, and a generated description string
•	Severity computation: severity = (policy.weight × recurrenceCount > 3 ? 'critical' : recurrenceCount > 1 ? 'high' : policy.weight > 3 ? 'medium' : 'low')
•	Returns { valid: Boolean, violations: [{ policyId, policyTitle, severity, description }] } to Module 1
Audit Trail Service
•	POST /api/internal/audit/log (SERVICE_KEY auth): called by Module 1 and Module 3 on every significant action. Receives { userId, action, entityType, entityId, module, before, after, ipAddress }
•	Computes tamper-evidence hash: SHA-256(userId + action + entityId + timestamp) using Node.js built-in crypto module; stores the hex digest in the hash field
•	Writes the m2_audit_logs document as an insert-only operation — updates and deletes on this collection are blocked at the application layer to preserve immutability
•	Audit query service: compound MongoDB index on (userId, action, timestamp) for efficient filtering; text index on action and entityType fields for keyword search; pagination using cursor-based approach for large log datasets
•	Audit export: streams matching records through a json2csv transformer and returns the output as a downloadable CSV file with Content-Disposition header
Risk Assessment Service
•	Risk CRUD: on every create or update, recompute compositeScore = likelihood × impact and store it; this pre-computed value is read by Module 3's AI/ML feature pipeline
•	GET /api/risks/score/:deptId (SERVICE_KEY auth): aggregates all active m2_risks for a department; returns { maxScore, avgScore, level, breakdown: [{ riskId, title, compositeScore }] }; this endpoint is called by Module 3 every 24 hours as input for the Random Forest model
Authentication, Security & Logging (Module 2)
•	Same validateJWT and requireRole middleware as Module 1; same JWT_SECRET env var
•	SERVICE_KEY middleware on all /api/internal/* routes
•	express-validator on all public endpoints; express-rate-limit; CORS; Helmet.js
•	Morgan HTTP logger; Winston structured JSON logger for errors and compliance events

3.2 Pages to be Created — Module 2

Module 2 frontend is built with HTML, CSS, and JavaScript (no framework). The following 10 pages are required:

Page / Screen	Purpose	UI Components	User Actions	Data Displayed
Policy Library	Browse, search, and manage all organisational policies	Search input, Category filter dropdown, Status filter buttons, Policy card grid (title / category / version / effective date / status), Create Policy button (admin/manager only), Pagination	Search, filter by category or status, click card to view detail, create new policy	Policy name, category badge, version number, effective date, assigned department count, compliance rate percentage
Create / Edit Policy	Author a new policy or revise an existing one	Title input, Category select, Effective Date date-picker, Department multi-select checkboxes, Policy body textarea, Version notes textarea (on edit), Save Draft button, Submit for Review button, Cancel button	Fill all fields, select departments, save draft or submit for review; on edit: compare against prior version in side panel	On edit: current text pre-filled, version history panel, current version number and last editor
Policy Detail View	Read the full text and history of a published policy	Policy metadata header (title, category, status badge, version), Full policy body text, Version history accordion, Assigned Departments list, Compliance Rate badge, Download PDF button, Edit/Archive buttons (admin only)	View full text, expand version history entries, download PDF copy, edit or archive (admin only)	Policy content, effective date, version changelog entries, assigned departments, compliance rate for this policy
Compliance Dashboard	Real-time overview of the organisation's compliance status	Overall Compliance % KPI card, Violations Today counter, Open Violations counter, Per-department compliance table with progress bars and colour coding, Recent Violations feed (last 10 rows), Date range filter, Department filter dropdown	Filter by department or date range, click department row to filter violations below, click violation to open detail	Overall compliance %, per-department compliance %, violation counts by severity (Critical/High/Medium/Low), trend indicator arrows
Violation Registry	Full searchable and filterable list of all detected compliance violations	Filter form (severity multi-select, department dropdown, status dropdown, date range), HTML table (violation ID, policy name, linked decision, department, severity badge, detected date, status badge, Resolve button per row), Export CSV button, Pagination	Filter, sort, click row to open detail, resolve violation from list row, export filtered data to CSV	Violation ID, linked policy name, linked decision title, department, severity, detection date, resolution status, assigned owner
Violation Detail	Full context and resolution management for a single violation	Violation metadata header, Linked Policy card, Linked Decision card, Evidence description block, Resolution form (status dropdown, notes textarea, assign-owner user search), Comment thread, Audit history for this violation	Change resolution status, write resolution notes, assign remediation owner, add comments, escalate to risk register	Full violation context, linked policy text excerpt, linked decision summary, resolution history timeline, assigned owner
Audit Log Viewer	Search and browse the immutable system-wide audit trail	Advanced filter form (user dropdown, action type multi-select, entity type select, module select, date from/to), Results table (user, action, entity, module, timestamp, IP), Export CSV button, Pagination controls	Apply filters, search by keyword, export filtered results as CSV, click row to view entry detail	User name, action label, entity type and ID, source module, IP address, exact timestamp
Audit Entry Detail	Inspect every field of a single audit log entry including tamper proof hash	Metadata section (user, action, timestamp, IP, module), Before-state JSON display block, After-state JSON display block, Tamper hash string, Verification status badge (Verified / Warning), Copy Hash button	View full before/after snapshots, copy tamper hash to clipboard, navigate to the linked entity	Complete before/after state snapshots, SHA-256 hash value, verification status
Risk Register	Organisation-wide risk identification, scoring, and tracking	Risk table (title, category, likelihood, impact, composite score badge, owner, status, last updated), Add Risk inline form, Risk score matrix chart (likelihood vs impact grid with colour zones), Filter by category/status dropdowns	Add risk, edit risk scores, assign owner, update mitigation status, link to decision categories, delete risk (admin only)	All active risks with composite scores, owner names, mitigation status, score history trend
Compliance Reports	Generate and download compliance summary reports	Parameter form (Date From/To inputs, Department multi-select, Format radio: PDF/CSV), Generate button, Preview section (row counts, date range confirmation), Past Reports table (date, description, format badge, Download button)	Configure parameters, generate report, preview scope, download in chosen format, re-download past reports	Generated report scope summary; past report list with date, type, format, and download link

3.3 Database Design — Module 2

Module 2 owns five collections. It reads the shared users and departments collections. m2_audit_logs is written by all three modules via the internal service endpoint.

Collection: m2_policies
Field	Type	Constraints	Description
PK _id	ObjectId	Auto	Unique policy ID
title	String	Required, Indexed	Policy display name
content	String	Required	Full policy text
category	String	Required, Indexed	Matched against decision category in compliance validation
version	Number	Default: 1	Auto-incremented on every edit; latest version always on this document
status	String	Indexed	draft | under_review | published | archived
effectiveDate	Date	Indexed	Policy activates after this date; checked by node-cron each morning
expiryDate	Date	Nullable	Policy auto-archives after this date if set
assignedDepts	Array		[ObjectId] ref: departments — departments this policy governs; used in compliance validation
assignedRoles	Array		[String] roles this policy applies to
weight	Number	Default: 1, range 1-5	Policy importance weight; used in violation severity computation
FK createdBy	ObjectId	Ref: users	Policy author
publishedAt	Date	Nullable	Timestamp when status changed to published
createdAt	Date	Auto	
updatedAt	Date	Auto	

Collection: m2_policy_versions
Field	Type	Constraints	Description
PK _id	ObjectId	Auto	
FK policyId	ObjectId	Ref: m2_policies, Indexed	Parent policy document
versionNumber	Number	Required	Sequential version number for this policy
content	String	Required	Full text snapshot of the policy at this version
versionNote	String		Description of what changed in this version
FK changedBy	ObjectId	Ref: users	User who made the change
createdAt	Date	Auto	Snapshot timestamp

Collection: m2_violations
Field	Type	Constraints	Description
PK _id	ObjectId	Auto, Indexed	Unique violation ID; count per-department read by Module 3 AI/ML
FK policyId	ObjectId	Ref: m2_policies, Indexed	Which policy was breached
FK decisionId	ObjectId	Ref: m1_decisions, Indexed	Cross-module: which decision triggered this violation
FK department	ObjectId	Ref: departments, Indexed	Department of the offending decision; used in M3 KPI grouping
severity	String	Indexed	low | medium | high | critical (computed from policy.weight and recurrence)
description	String	Required	Auto-generated text describing which rule was breached
status	String	Indexed	open | in_remediation | resolved | false_positive
FK assignedOwner	ObjectId	Ref: users, Nullable	User responsible for remediation
detectedAt	Date	Auto, Indexed	When the violation was created
resolvedAt	Date	Nullable	When status changed to resolved
FK resolvedBy	ObjectId	Ref: users, Nullable	
resolutionNotes	String	Nullable	

Collection: m2_audit_logs  [Written by all three modules via internal API]
Field	Type	Constraints	Description
PK _id	ObjectId	Auto	Unique log entry ID
FK userId	ObjectId	Ref: users, Indexed	User who performed the action
action	String	Required, Indexed	DECISION_SUBMITTED | DECISION_APPROVED | POLICY_PUBLISHED | VIOLATION_RESOLVED | REPORT_GENERATED | etc.
module	String	Indexed	m1 | m2 | m3
entityType	String	Required	decision | policy | violation | user | risk | report
FK entityId	ObjectId	Indexed	ID of the affected entity
ipAddress	String		Client IP address
before	Object	Nullable	State snapshot of the entity before the action
after	Object	Nullable	State snapshot of the entity after the action
hash	String	Required	SHA-256(userId + action + entityId + timestamp) — tamper-evidence token
timestamp	Date	Auto, Indexed	Precise moment of the action; never editable

Collection: m2_risks
Field	Type	Constraints	Description
PK _id	ObjectId	Auto	
title	String	Required	Risk short title
description	String	Required	Detailed risk description
category	String	Indexed	Operational | Compliance | Strategic | Financial
likelihood	Number	Required, 1–5	Probability of risk occurring
impact	Number	Required, 1–5	Consequence severity if risk occurs
compositeScore	Number	Auto-computed	likelihood × impact (range 1–25); read by Module 3 AI/ML pipeline as a feature
FK department	ObjectId	Ref: departments, Indexed	Owning department; grouped in Module 3 risk scoring
FK owner	ObjectId	Ref: users	Risk owner responsible for mitigation
status	String		identified | in_mitigation | mitigated | accepted
createdAt	Date	Auto	
updatedAt	Date	Auto	

Module 2 Cross-Module Relationships
•	m2_violations.decisionId is a cross-module FK to m1_decisions._id (Module 1) — created whenever Module 1 submits a decision and this module detects a breach
•	m2_violations.department and m2_risks.department are FK references to departments._id (shared collection) — used by Module 3 for per-department compliance and risk analytics
•	m2_audit_logs receives entries from Module 1 (on every decision state change) and from Module 3 (on report generation and AI/ML retraining) — all via POST /api/internal/audit/log with SERVICE_KEY
•	m2_risks.compositeScore is read by Module 3 every 24 hours as an input feature for the Random Forest risk classifier
•	m2_policies.assignedDepts controls which Module 1 decisions are subject to compliance validation

3.4 REST API — Module 2

Policy Endpoints
Method	Endpoint	Description	Request Body	Response | Auth
GET	/api/policies	List policies with filters	Query: status, category, page	200 { policies[], total } | JWT
POST	/api/policies	Create policy draft	{ title, content, category, effectiveDate, assignedDepts[] }	201 { policy } | JWT + admin/manager
GET	/api/policies/:id	Policy detail with version history	Param: id	200 { policy, versions[] } | JWT
PUT	/api/policies/:id	Update policy; auto-versions old content	{ content, versionNote }	200 { policy } | JWT + admin
POST	/api/policies/:id/publish	Publish policy	{}	200 { policy } | JWT + admin
POST	/api/policies/:id/archive	Archive policy	{ reason }	200 { policy } | JWT + admin
GET	/api/policies/:id/versions	List all version history	Param: id	200 { versions[] } | JWT

Compliance Endpoints
Method	Endpoint	Description	Request Body	Response | Auth
GET	/api/compliance/status	Org-wide compliance summary	Query: dept, from, to	200 { overall%, deptBreakdown[] } | JWT + manager
GET	/api/compliance/violations	List violations	Query: severity, dept, status, page	200 { violations[], total } | JWT
GET	/api/compliance/violations/:id	Violation detail	Param: id	200 { violation, policy, decision } | JWT
PUT	/api/compliance/violations/:id/resolve	Resolve violation	{ resolution, notes, assignedOwner }	200 { violation } | JWT + compliance_officer
GET	/api/compliance/score/:deptId	Department compliance score	Param: deptId	200 { score, history[] } | JWT

Internal Service Endpoints (SERVICE_KEY Auth)
Method	Endpoint	Description	Request Body	Response | Auth
POST	/api/internal/compliance/validate	Validate decision against published policies (called by M1)	{ decisionId, category, dept }	200 { valid, violations[] } | SERVICE_KEY
POST	/api/internal/audit/log	Write immutable audit entry (called by M1 and M3)	{ userId, action, entityType, entityId, module, before, after, ip }	201 { log } | SERVICE_KEY
GET	/api/risks/score/:deptId	Dept risk features for AI/ML (called by M3)	Param: deptId	200 { maxScore, avgScore, level, breakdown[] } | SERVICE_KEY

Audit & Risk Endpoints
Method	Endpoint	Description	Request Body	Response | Auth
GET	/api/audit/logs	Query audit logs with filters	Query: userId, action, module, from, to, page	200 { logs[], total } | JWT + admin
GET	/api/audit/logs/:id	Single audit entry with hash verification	Param: id	200 { log, hashValid } | JWT + admin
GET	/api/audit/export	Export log range as CSV	Query: from, to, userId, action	200 CSV File Download | JWT + admin
GET	/api/risks	List all risks	Query: status, category, dept, page	200 { risks[], total } | JWT + manager
POST	/api/risks	Create risk entry	{ title, description, likelihood, impact, department, category }	201 { risk } | JWT + manager
PUT	/api/risks/:id	Update risk; recompute composite score	{ likelihood, impact, status, mitigationPlan }	200 { risk } | JWT + manager
